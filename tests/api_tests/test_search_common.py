...
class TestSearchCommon(MarqoTestCase):
    ...
    def test_tensor_search_context_zero_vector(self):
        context = {"tensor": [{"vector": [0, ] * 384, "weight": 1},
                          {"vector": [0, ] * 384, "weight": 2}]}
        for index_name in [self.structured_text_index_name, self.unstructured_text_index_name]:
            with self.assertRaises(MarqoWebError) as e:
                res = self.client.index(index_name).search(q=None, context=context)
                print(res)
                self.assertEqual(res["status_code"], 400)
                self.assertEqual(res["type"], "invalid_request")
                self.assertEqual(res["code"], "bad_request")
                self.assertIn("Magnitude of combined query and context vectors cannot be zero. "
                              "If you want to pass a zero vector, please set normalizeEmbeddings = False during index creation",
                              res["message"])

    def test_tensor_search_context_vector_weight_zero(self):
        context = {"tensor": [{"vector": [1, ] * 384, "weight": 0},
                          {"vector": [1, ] * 384, "weight": 0}]}
        for index_name in [self.structured_text_index_name, self.unstructured_text_index_name]:
            with self.assertRaises(MarqoWebError) as e:
                res = self.client.index(index_name).search(q=None, context=context)
                print(e)
    ...
    def test_tensor_search_query_vector_weight_zero(self):
        for index_name in [self.structured_text_index_name, self.unstructured_text_index_name]:
            with self.assertRaises(MarqoWebError) as e:
                res = self.client.index(index_name).search(q = {"some query": 0})
                self.assertEqual(res["status_code"], 400)
                self.assertEqual(res["type"], "invalid_request")
                self.assertEqual(res["code"], "bad_request")
                self.assertIn("Magnitude of combined query and context vectors cannot be zero. "
